...
## Load data
```{r}
data <- read.csv("../../data/data_listenerside/data_pilot2.csv")
...
fit <- brm(
  formula = response_condition ~ condition + (1 + condition | submission_id) + (1 + condition | item_id),
  family  = categorical(),
  data    = data_brm,
  chains  = 4,
  iter    = 4000,
...
data_brm |> 
  ...
  group_by(condition) |> 
  summarize(
    post_prob_hypothesis_conj = mean(match > `non-match-1` & match > `non-match-2`),
    post_prob_hypothesis_sum = mean(match > `non-match-1` + `non-match-2`)
  )
```
