...
First we try with GLM based analysis. We could for each conditions fit and non-fit.
...
fit_tests <- fit_summary %>%
  ...
  ungroup() %>%
  select(condition, n, n_fit, prop_fit, p_value, ci_low, ci_high)

fit_tests
```

Finally we will try a ordinal regression analysis:
```{r}
...
fixef_ord_or
```
A Bayesian cumulative ordinal regression revealed that the low condition reliably shifted responses toward lower categories (OR = 0.41, 95% CrI [0.20, 0.83]).
The high condition showed a positive but uncertain effect (OR = 1.77, 95% CrI [0.85, 3.80]).
These results indicate strong evidence that the low-information trials pull participants toward selecting “low,” while the high-information trials show only weak evidence for shifting choices toward “high.”
