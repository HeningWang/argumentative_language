...
\mf{Not for the abstract, but possibly for the long paper, I would say.}
The probability of observing the full results from an exam is then proportional to the product of the Binomial probability of each student's results: \mf{check: why $\propto$ here? isn't it actually $=$?}
  %
...
Consequently, for the models below, the assumed hypotheses are: \mf{elaborate on why exactly these numbers?}
...
\end{equation}
%
\noindent where $P( \interpret{u} \mid H )$ is the probability that utterance $u$ is \emph{true} (rather than, e.g., \textit{produced}) given hypothesis $H$. $P(\interpret{u} \mid H)$ can be calculated based on the literal semantics, with with $P(s \mid \gamma_H)$ is defined in Eq.~\eqref{eq:stateprob}, as:
\begin{equation}
...
First, the vanilla RSA model has a comparatively poor predictive accuracy.
...
Fitting and comparing models on the different matrix-size conditions independently also reveals interesting differences.
\mf{Reader may wonder whether (i) models were trained on all conditions and evaluated on pairwise comparisons, or (ii) trained on only data from the pair to be compared.}
...
\mf{It would be great to be able to say whether the best fitting model is in some absolute sense good enough. We could compute a posterior predictive $p$-value, for example.}
