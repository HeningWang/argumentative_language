...
Fitting and comparing models on the different matrix sizes conditions independently also reveals interesting differences. For instance, the maximin model is better than the model-free one for the 5$\times$12 condition, while the opposite is true in the 11$\times$6 and 5$\times$6 conditions (the models are not credibly different in the 11$\times$ 12 condition). Since each model encodes an account of argumentative strength that we do not expect to interact with the matrix size condition, this differences in performance might reflect a failure of some linking modeling assumption, e.g., the Binomial likelihood for student results.
