...
Our main findings are that the log-likelihood ratio model captures the data worse than other formalizations of argumentative strength, and that there are patterns in the data not captured by any of the models we considered. 
...
For practical purposes, we fixed values for these hypotheses as follows:\footnote{Treating values for the hypotheses as free variables leads to issues of unindentifiability. Choice of different values largely led to qualitatively similar results.}
...
Finally, participants chose `none : all : right' more often than predicted by any model, for both frame condition. For example, in the high frame condition, it is chosen for exam results where all students got all answers wrong.
