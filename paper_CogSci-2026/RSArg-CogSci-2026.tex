...
\begin{figure*}[t]
  ...
  \caption{
    ...
    \fc{When only one condition is represented the bar becomes very wide, e.g., `all|most|wrong' 11x6 - this is fine but maybe visually confusing?}
  }
...
The posterior predictive checks for each model also show systematic patterns in how they correctly fit or depart from the data, which suggests ways in which the corresponding accounts of argumentative strength succeed or fail in this experimental setup. We briefly discuss this for each model. 
...
\paragraph{vanilla RSA} Since this model does not take into account the argumentative goal of participants, it overpredicts the probability of informative but weak signals, such as `some|most|wrong' and `some|all|wrong' in the high framing condition. It also underpredicts the frequency of signals that can be exploited for paltering, such as `most|most|right' and `most|all|right'.

\paragraph{lr-argstrength} The simple lr-argstrength and its pragmatic variation make very similar predictions.
\begin{comment}
Looking at the datapoints that the different models fail to predict reveals some illuminating patterns. For the lr-argstrength model, we look at the signals that were produced most often by participants for a certain observation but which are not on the Pareto frontier of informativity/lr-argstrength, which means that no value of $\beta$ could account of them. These are:
\begin{itemize}
...
\paragraph{Model free argstrength} In many cases, mf-argstrength makes similar predictions as maximin-argstrength. However, for 
...
\paragraph{Overall} All models overpredict the production of `(some,most,all)|(some,most)|wrong' in the low condition and `(some,most,all)|(some,most)|right' in the high condition.  All models also underpredict the frequency of `some|some|wrong' in the low framing condition and `some|some|right' in the high framing condition (e.g., for state $[1,1,1,0,0]$, because they distribute their predictions across a larger set of utterance, mostly using adjective `wrong' in the high framing condition and `false' in the low framing condition. Both of these patterns can be explained by observing that all measures of argumentative strengths we defined are truth-functional, and therefore cannot capture the connotation of conveying the same content in terms of `right' or `false'.
...
Fitting and comparing models on the different matrix sizes conditions reveals interesting differences. For instance, the maximin model is better than the model-free one for the 5$\times$12 condition, while the opposite is true in the 11$\times$6 and 5$\times$6 conditions (the models are not credibly different in the 11$\times$ 12 condition). We leave a detailed analysis of these patterns to future work.
